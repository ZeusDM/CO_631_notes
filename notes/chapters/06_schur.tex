...
\begin{definition}[Kostka numbers]
    ...
    The \vocab{Kostka number} \(K_{\lambda \mu}\) is the number of standard Young tableaux of shape \(\lambda\) and weight \(\mu\).
\end{definition}
...
\begin{theorem} \label{thm:h-e-sum-kostka-schur}
    ...
    and
    \begin{equation}
        e_\mu = \sum_{\lambda} K_{\lambda \mu} s_{\tilde\lambda}.
    \end{equation}
\end{theorem}
...
\begin{proof}
    ...
    \begin{align}
        h_\mu
        &= h_{\mu_1} h_{\mu_2} \cdots h_{\mu_\ell} \\
        &= s_{\varnothing} h_{\mu_1} s_{\varnothing} h_{\mu_2} \cdots s_{\varnothing} h_{\mu_\ell},
    \end{align}
